...
Node* Parser::minorError(string type,string msg)
{
  minors.push_back(MinorError(type,msg,filename));
  minors[minors.size()-1].line = line_num;
  Node* n = NewNode(NodeType::deadend,msg);
  return n;
}
...
Node* Parser::parseExpr(const vector<Token>& tokens)
{
    ...
    if(tokens.size()==1)
    {
        ...
        NewNode(NodeType::deadend,"SyntaxError: Invalid Syntax");
    }
    ...
    if(tokens[tokens.size()-1].type==END_LIST_TOKEN)// evaluate indexes or lists
    {
        ...
        else
        {
            ...
            {
                if(t.size()==0)
                    return minorError("SyntaxError","Error expected an element before ',' ");
            ...
            if(t.size()==0)
                return minorError("SyntaxError","Error expected an element after ',' ");
            ast->childs.push_back(parseExpr(t));
            t.clear();
            return ast;
        }
    ...
    if(!multiline)
        return minorError("SyntaxError","Invalid syntax: "+expr);
    else
...
if(tokens[0].type==TokenType::KEYWORD_TOKEN && tokens[0].content=="private")
{
    ...
    tokens.erase(tokens.begin());
    if(!inclass)
        return minorError("SyntaxError","Error use of keyword private outside class!");
    ...
    else if(tokens[0].type==TokenType::KEYWORD_TOKEN && tokens[0].content=="function")
    {
        ...
        return minorError("SyntaxError","Invalid use of keyword private\n");
}
else if(tokens[0].type==TokenType::KEYWORD_TOKEN && tokens[0].content=="public")
{
    ...
    tokens.erase(tokens.begin());
    if(!inclass)
        return minorError("SyntaxError","Error use of keyword public outside class!");
    if(tokens[0].type==TokenType::KEYWORD_TOKEN && tokens[0].content=="var")
    {
    ...
    else if(tokens[0].type==TokenType::KEYWORD_TOKEN && tokens[0].content=="function")
    {
        ...
        return minorError("SyntaxError","Invalid use of keyword public");
}
...
if(tokens[0].type== TokenType::KEYWORD_TOKEN  && tokens[0].content=="var")
{
    ...
    if(expr[0].type==KEYWORD_TOKEN && expr[0].content == "yield")
    {
        ...
        if(expr.size()==0)
          return minorError("SyntaxError","Error expected expression after return keyword");
        Node* n = NewNode(NodeType::line ,to_string(tokens[0].ln));
...
if(tokens[0].type== TokenType::KEYWORD_TOKEN && tokens[0].content=="yield")
    ...
    vector<Token> t = {tokens.begin()+1,tokens.end()};
    if(t.size()==0)
        return minorError("SyntaxError","Error expected expression after yield keyword");
    Node* n = NewNode(NodeType::line,to_string(tokens[0].ln));
    ast->childs.push_back(n);
    ast->childs.push_back(parseExpr(t));
    return ast;
        ...
        if(tokens[k].type== TokenType::NEWLINE_TOKEN)
        {
            ...
            if(ast->childs[1]->type!=NodeType::ID)
            {
                ...
                else
                {
                    ...
                    if(substr(0,6,filename) == "file://")
                      tmp = filename.substr(7);
                    #ifdef _WIN32

                    #else
                        strncpy(path,tmp.c_str(),256);
                        dir = dirname(path);
                    #endif
                    ...
                    if(lex.hadErr)
                    {
                        line_num = K;
                        filename = F;
                        return minorError("SyntaxError","The imported file has errors.");
                    }
                    bool empty_file = false;
                    if(tokens.size() == 1 && tokens[0].type == EOP_TOKEN) //empty file
                    {
                      empty_file = true;
                    }
                    A = NewNode(NodeType::file);
